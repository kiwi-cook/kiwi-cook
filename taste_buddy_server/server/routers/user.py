...
from models.user import User, get_current_active_user, Token, authenticate_user, \
    ACCESS_TOKEN_EXPIRE_MINUTES, create_access_token, UserInDB
...
async def create_user(username: Annotated[str, Form()], password: Annotated[str, Form()]):
    ...
    return {"error": False, "response": write_client["users"]["users"].insert_one(user.model_dump())}
...
async def add_user_friend(
    ...
    write_client["users"]["users"].update_one({"username": current_user.username},
