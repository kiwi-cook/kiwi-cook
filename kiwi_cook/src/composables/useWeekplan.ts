  ...
  function generateWeekplan(
    ...
    const availableRecipes = recipeStore.recipes.filter((recipe) => {
      ...
      if (options.dietaryRestrictions && recipe.dietaryRestrictions) {
        matches &&= options.dietaryRestrictions.every((restriction) =>
        ...
        .map((mealType) => {
          ...
          for (let i = 0; i < availableRecipes.length; i++) {
            ...
            const usedIngredients = recipe.ingredients.filter((ri) =>
              ingredients.some((ui) => ui.ingredient.name === ri.ingredient.name)
            ).length
  ...
  function generateRandomWeekplan(days: number): {
    ...
    for (let i = 0; i < days; i++) {
      ...
      mealTypes.forEach((mealType) => {
        const randomRecipe =
          recipeStore.recipes[Math.floor(Math.random() * recipeStore.recipes.length)]
