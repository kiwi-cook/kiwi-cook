<template>
  <q-btn
    v-if="!result"
    class="q-mt-md llm-task-button"
    :class="{ loading: modelDownloading }"
    :loading="!taskFinished"
    @click="executeTask"
  >
    ...
    <template #loading>
      <div class="row items-center">
        ...
        <span class="q-ml-sm">
          {{
            modelDownloading
              ? `${$t('llm.downloading')} ${modelDownloadProgress}%`
              : $t('llm.processing')
          }}
        </span>
      </div>
...
import { watch, computed, defineEmits, defineProps, withDefaults } from 'vue'
...
const emit = defineEmits<{
  (e: 'on-output', result: unknown): void
