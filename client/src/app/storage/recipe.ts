...
export const useRecipeStore = defineStore('recipes-app', () => {
    ...
    const ingredientSuggestions = computed(() => {
        ...
        const randomItems: Ingredient[] = sharedRecipeStore.ingredients.filter(
            () => Math.random() < 0.5
        );

        const itemsFromSavedRecipes: Ingredient[] = savedRecipes.value.reduce(
            (items: Ingredient[], recipe: Recipe) => {
                const recipeItems: Ingredient[] = recipe
                    .getRecipeIngredients()
                    .map(
                        (item: RecipeIngredient) =>
                            sharedRecipeStore.ingredientMap[item.getId()]
                    );
        ...
        const itemIds = new Set(
            [...randomItems, ...itemsFromSavedRecipes].map((item: Ingredient) =>
                item.getId()
            )
        );
        return [...itemIds]
            .map((itemId: string) => sharedRecipeStore.ingredientMap[itemId])
            .filter((item: Ingredient) => typeof item !== 'undefined');
    });
    ...
    const recipeOfTheDay = computed(() => {
        ...
        const randSortedRecipesAsList = recipes.value.toSorted(
            () => getRandom() - 0.5
        );
    ...
    const savedRecipeStats = computed(() => {
        ...
        for (const savedRecipe of savedRecipesList) {
            ...
            keyValues.numberOfIngredients.push(
                savedRecipe.getRecipeIngredients().length
            );
    ...
    const savedRecipeMap = computed(() =>
        [..._savedRecipes.value].reduce(
            (recipes: { [id: string]: Recipe }, recipeId) => {
    ...
    const tags = computed(() => [
        ...new Set(
            recipes.value.reduce(
                (tags: string[], recipe: Recipe) => [
                    ...tags,
                    ...(recipe.props.tags ?? []),
                ],
                []
    ...
    async function prepareStore() {
        return useSharedRecipeStore()
            .prepareStore()
            .then(() => getCachedItem<string[]>('savedRecipes', []))
            .then((savedRecipesCache: CachedItem<string[]>) =>
                setSavedRecipes(savedRecipesCache.value ?? [])
            )
            .then(() => prepareSearch());
    }
    ...
    function updateRecipePredictions() {
        const predictedRecipes = simpleRecipeSuggestion(
            config.value.numberOfPredictions
        );
