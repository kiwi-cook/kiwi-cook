    ...
    actions: {
        ...
        async prepare() {
            ...
            getCachedItem<Item[]>('items')
                .then(async (cachedItems: { value: Item[] | null, isOld: boolean }) => {
                    this.setLoadingState('getCachedItems')
                    if (cachedItems.isOld || cachedItems.value === null) {
                    ...
                    return this.replaceItems(cachedItems.value.map((item: Item) => itemFromJSON(item))).then((items) => {
                        this.finishLoading('getCachedItems')
                        return items
                    })
                })
                .then(() => {
                    // get all recipes
                    getCachedItem<Recipe[]>('recipes').then(async (cachedRecipes: {
                        value: Recipe[] | null,
                        isOld: boolean
                    }) => {
                        this.setLoadingState('getCachedRecipes')
                        if (cachedRecipes.isOld || cachedRecipes.value === null) {
                            return this.fetchRecipes().then((recipes) => {
                                this.finishLoading('getCachedRecipes')
                                return recipes
                            })
                        }
                        return Promise.all(cachedRecipes.value.map((recipe: Recipe) => recipeFromJSON(recipe))).then((recipes: Recipe[]) => {
                            this.finishLoading('getCachedRecipes')
                            return this.replaceRecipes(recipes, false)
                        })
                    })
                })
                .then(() => {
                    // get saved recipes
                    getCachedItem<string[]>('savedRecipes').then((cachedSavedRecipes: {
                        value: string[] | null,
                        isOld: boolean
                    }) => {
                        this.setLoadingState('getCachedSavedRecipes')
                        if (!cachedSavedRecipes.isOld && cachedSavedRecipes.value !== null) {
         ...
         * @param updateCache
         */
        replaceRecipes(recipes: Recipe[], updateCache = true) {
